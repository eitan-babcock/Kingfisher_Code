...
public:
	...
	: n()
	{
		...
		filter_flag_sub = n.subscribe("filter_flag", 10, &PositionFilterLong::FilterFlag_Callback, this);
		unfiltered_cam_pose_sub = n.subscribe("kingfisher_vision_long/deckPose", 10, &PositionFilterLong::imageCB, this);
		unfiltered_heading_sub = n.subscribe("kingfisher_vision_long/unfilteredHeading", 10, &PositionFilterLong::Heading_Callback, this);
		
		filtered_pose_pub = n.advertise<geometry_msgs::Twist>("filtered_pose_long", 10);
		frame_drop_counter_pub = n.advertise<std_msgs::Int32>("frame_drop_counter_long", 10);
		filtered_heading_pub = n.advertise<std_msgs::Float32>("filtered_heading_long", 10);
...
/*		else if(
	...
	void Transform(void) {
		...
		//filtered_pose.linear.x = 0.866*filtered_cam_pose.linear.z+0.5*filtered_cam_pose.linear.x;
		//filtered_pose.linear.y = -1*filtered_cam_pose.linear.y;
		//filtered_pose.linear.z = 0.5*filtered_cam_pose.linear.z-0.866*filtered_cam_pose.linear.x;

		// zero degree offset
		filtered_pose.linear.x = 1*filtered_cam_pose.linear.z+0*filtered_cam_pose.linear.x;
		filtered_pose.linear.y = -1*filtered_cam_pose.linear.y;
		filtered_pose.linear.z = 0*filtered_cam_pose.linear.z-1*filtered_cam_pose.linear.x;
...
{
	ros::init(argc, argv, "position_filter_long");
	PositionFilterLong pfs;
